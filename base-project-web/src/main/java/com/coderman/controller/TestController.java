...
public class TestController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "currentPage",dataType = SwaggerConstant.DATA_INT,value = "当前页",paramType = SwaggerConstant.PARAM_FORM,required = true),
            @ApiImplicitParam(name = "pageSize",dataType = SwaggerConstant.DATA_INT,value = "每页显示条数",paramType = SwaggerConstant.PARAM_FORM,required = true),
    })
    @ApiReturnParams({
            @ApiReturnParam(name = "ResultVO",value = {"code","msg","result"}),
            @ApiReturnParam(name = "PageVO",value = {"code","total","dataList"}),
            @ApiReturnParam(name = "UserVO",value = {"username","nickname"})
    })
    public ResultVO<PageVO<List<UserVO>>> page(Integer currentPage,Integer pageSize){

        PageVO<List<UserVO>> page =  new PageVO<>();
        page.setDataList(Arrays.asList(
